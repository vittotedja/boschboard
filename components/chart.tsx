...
const groupDataByTimeframe = (data: DataItem[], timeframe: Timeframe): AggregatedData[] => {
  ...
  const getBucket = (timestamp: Date) => {
    ...
    else if (timeframe === "5m") date.setMinutes(Math.floor(date.getMinutes() / 5) * 5, 0, 0);
  ...
  return Object.entries(grouped).map(([time, values]) => {
    ...
    const stdDev = Math.sqrt(values.map(v => Math.pow(v - average, 2)).reduce((a, b) => a + b, 0) / values.length);
    return {
      time,
      open: values[0],
...
const TimeSeriesChart: React.FC = () => {
  ...
  const [initialRange, setInitialRange] = useState<[Date | null, Date | null]>([null, null]);
    ...
    if (data.length > 0) {
      ...
      switch (timeframe) {
        case "1m":
          ...
          break;
        case "5m":
          startTimestamp = new Date(latestTimestamp.getTime() - 50 * 5 * 60 * 1000);
          break;
        case "1h":
          startTimestamp = new Date(latestTimestamp.getTime() - 50 * 60 * 60 * 1000); 
          break;
        case "1d":
          startTimestamp = new Date(latestTimestamp.getTime() - 50 * 24 * 60 * 60 * 1000); 
          break;
        case "tick":
        default:
      ...
      <div className="h-96">
        {timeframe === "tick" ? (
          <Plot
            ...
            layout={{
              title: "Tick Chart",
              xaxis: { type: "date", range: getXRange(),rangeslider: {
                visible: false
              }},
          ...
          />
            ...
            data={[
              ...
              ...(showBollingerBands
                ? [
                    ...
                    {
                      x: aggregatedData.map((d) => d.time).concat(aggregatedData.map((d) => d.time).reverse()),
            ...
            layout={{
              xaxis: { type: "date", range: getXRange(), rangeslider: { visible: false } },
