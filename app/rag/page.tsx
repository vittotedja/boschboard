...
{/* Sidebar */}
      <div className="w-64 border-r border-gray-200 flex flex-col">
        <div className="p-4">
          <Button onClick={createNewConversation} className="w-full bg-sky-950 hover:bg-sky-900 text-white">
            ...
            {conversations.map((conv) => (
              <Button
                ...
                className={`w-full justify-start text-left ${
                  conv.id === currentConversationId ? "bg-sky-250 text-sky-950" : "text-sky-950 hover:bg-gray-100"
                }`}
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="p-4 border-b border-gray-200 flex justify-between items-center">
          <h1 className="text-xl font-semibold text-sky-950">BoschBoard Knowledge Assistant</h1>
          <Button
            variant="ghost"
            size="icon"
          ...
          <div className="space-y-6 max-w-3xl mx-auto">
            {currentConversation.messages.length === 0 ? (
              <div className="text-center py-12">
                ...
                <h2 className="text-xl font-semibold text-sky-950 mb-2">How can I help you with Rexroth today?</h2>
                <p className="text-gray-600 max-w-md mx-auto">
                  Ask me anything about Rexroth products, specifications, or applications.
                </p>
              </div>
            ) : (
                ...
                <div key={index} className={`flex ${message.role === "user" ? "justify-end" : "justify-start"}`}>
                  <div
                    className={`flex gap-3 max-w-[80%] ${
                      message.role === "user" ? "bg-sky-200 text-black" : "text-sky-950"
                    } p-4 rounded-lg`}
                  >
                    <div className="flex-shrink-0">
                      {message.role === "user" ? <User className="h-5 w-5" /> : <BrainCircuit className="h-5 w-5" />}
                    </div>
